...
class QualityResponseFlagOnly(BaseModel):
    flags: dict[str, Any] | None = Field(
        default=None,
        description="Все флаги с подробностями",
    )

class HeadResponse(BaseModel):
    rows : list[dict[str, Any]] = Field(
        ...,
        description = "Первые n-строк датасета",
    )
...
@app.post(
    ...
    response_model=QualityResponseFlagOnly,
    tags=["quality"],
    summary="Оценка качества по флагам",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityResponseFlagOnly:
    start = perf_counter()
    ...
    flags_bool: dict[str, bool] = {
        "too few rows" : flags_all["too_few_rows"],
        "too many columns" : flags_all["too_many_columns"],
        "too many missing" : flags_all['too_many_missing'],
        "has high cardinality categoricals" : flags_all['has_high_cardinality_categoricals'],
        "has constant columns" : flags_all['has_constant_columns'],
    ...
    # # if not df.empty:
    # #     message = "Датасет не пустой - можно работать"
    # # else:
    # #     message = "Датасет пустой - работа дальше невозможна"
    ...
    return QualityResponseFlagOnly(
        flags=flags_all
    )
    # return QualityResponse(
    #     ok_for_model=ok_for_model,
...
async def head(file: UploadFile = File(...), n : int = 5) -> HeadResponse:
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        # content_type от браузера может быть разным, поэтому проверка мягкая
        # но для демонстрации оставим простую ветку 400
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
